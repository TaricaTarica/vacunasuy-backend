...
@Named("graficaVacunaBean")
@RequestScoped
public class GraficaVacunaBean  implements Serializable {
	...
	@EJB
	private ReservaNegocioLocal reservaLocal;
    ...
    public void init() {
		...
		createScatterModel();
		createLineModel2();
		reservas = reservaLocal.obtenerReservas();
		registros = registroLocal.listarRegistros();
	}
	
	//////////////Aca creamos Grafica de vacunas en el tiempo
	
	public void createLineModel2() {
		List<DTReserva> reservas = reservaLocal.obtenerReservas();
		List<RegistroVacuna> registros = registroLocal.listarRegistros();
    	...
    	if(registros.isEmpty()) {
    		System.out.println("no hay vacunados");
    	}else {
    		for(RegistroVacuna r:registros) {
    			if(r.getFecha()!=null) {
    				
    				if(r.getFecha().getMonth().getValue()==0) {
    					enero++;
    				}
    				if(r.getFecha().getMonth().getValue()==1) {
    					febrero++;
    				}
    				if(r.getFecha().getMonth().getValue()==2) {
    					marzo++;
    				}
    				if(r.getFecha().getMonth().getValue()==3) {
    					abril++;
    				}
    				if(r.getFecha().getMonth().getValue()==4) {
    					mayo++;
    				}
    				if(r.getFecha().getMonth().getValue()==5) {
    					junio++;
    				}
    				if(r.getFecha().getMonth().getValue()==6) {
    					julio++;
    				}
    				if(r.getFecha().getMonth().getValue()==7) {
    					agosto++;
    				}
    				if(r.getFecha().getMonth().getValue()==8) {
    					setiembre++;
    				}
    				if(r.getFecha().getMonth().getValue()==9) {
    					octubre++;
    				}
    				if(r.getFecha().getMonth().getValue()==10) {
    					noviembre++;
    				}
    				if(r.getFecha().getMonth().getValue()==11) {
    					diciembre++;
    				}
    			}
    		}
	...
	////////////Aca arriba creamos Grafica de vacunas en el timepo
	...
	private List<DTReserva> reservaListar(){
		return reservaLocal.obtenerReservas();	
		
	}
	@SuppressWarnings("unused")
	private List<RegistroVacuna> registrosListar(){
		return registroLocal.listarRegistros();
	...
	public void createBarModel() {
	 ...
	 public void createRadarModel() {
	        ...
	        radarDataSet.setLabel("Casos sospechosos");
