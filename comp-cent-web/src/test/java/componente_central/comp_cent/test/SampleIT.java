package componente_central.comp_cent.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "comp-cent-ear.ear");

        // Current directory is the root of the "comp-cent-web" project. Go up one level
        // and enter the "comp-cent-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../comp-cent-ejb/target/comp-cent-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "comp-cent-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/comp-cent-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "comp-cent-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("componente_central.comp_cent.test");
